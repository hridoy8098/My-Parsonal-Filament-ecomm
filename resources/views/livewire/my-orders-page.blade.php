...
@php
  ...
  if($order->status == 'delivered'){
    $status = '<span class="bg-green-800 py-1 px-3 rounded text-white shadow">Delivered</span>';
  }
  ...
  if($order->payment_status == 'paid'){
    $payment_status = '<span class="bg-green-900 py-1 px-3 rounded text-white shadow">Paid</span>';
  }
...
<tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key="{{ $order->id }}">
    ...
    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
        <a href="/my-orders/{{ $order->id }}" class="bg-slate-600 text-white py-2 px-4 
        rounded-md hover:bg-slate-500">View Details</a>
    </td>
</tr>
@endforeach
